...
*   [X] `eventReg.jpg` — replaced with a real screenshot of the actual Event Registration Platform admin dashboard, supplied by the user. Screenshot contained visible-looking PII (NRIC numbers, phone numbers, an email, a company name) in the attendee rows; flagged to user before use, confirmed as test/dummy data from a dev event, safe to publish as-is. Converted from pasted PNG, compressed to JPEG (quality 90, ~170KB). Same filename, no import change needed.
...
**`LaptopFrame`/`PhoneFrame` cropping bug (2026-07-18):** `ProjectCard.tsx`'s shared `deviceImageBaseClasses` used `object-cover`, which crops any screenshot whose aspect ratio doesn't exactly match the frame's fixed ratio (16:10 laptop, 9:18 phone). Checked every screenshot in `public/images/` — only the hand-generated `travelSaas.jpg` matched exactly; the rest ranged 1.55–1.81 (laptop) and were all getting silently cropped (e.g. the UV Print Editor's sidebar/toolbar were cut off). Fixed by switching to `object-contain`, so every screenshot now displays in full with letterboxing (which reads as an intentional bezel against the frame's black background) instead of losing content. This was a site-wide bug, not isolated to the two newly swapped images.

**`birdApp.jpg` bird photo replacement (2026-07-20):** User provided a real screenshot of the TJIRP app's bird list screen but wanted the 7 bird photo thumbnails removed, since those are the client's original proprietary images. Located exact pixel coordinates of each thumbnail via background/content edge-detection scanning (all consistently `x: 38–227`, row heights ~166px), then replaced each with a simple flat-illustration bird silhouette (PIL-drawn, varied natural-toned backgrounds per row) — avoids any rights/copyright ambiguity while keeping the real app UI (nav, list layout, species names) intact. Same filename (`birdApp.jpg`), no import change needed.